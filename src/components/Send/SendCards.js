...
const[lines, setLines]=useState(maxLines);
  ...
  const [maxLinesReached, setMaxLinesReached] = useState(false);
  ...
  const handleEdit = () => {
    ...
    {    
        ...
        const cardMessage = edit.editData.cardMessage;
        const cardText =  cardMessage.replace(/<[^>]*>| /g, '')
        console.log(cardMessage);
        console.log(cardText);
        console.log('length',cardText.length);
        const lines = cardMessage.split(/<\/?p>/).filter(line => line.trim() !== '');
        console.log("Number of lines:", lines.length);
        setLines((maxLines)-(lines.length));
        setRemainingCharacters(maxCharacterLimit-cardText.length);    
    }
  };
  ...
  //       totalCharacters += op.insert.length-1;
  ...
  //       const quill=quillInstance.deleteText(totalCharacters - 1, 1);
  //       console.log("----",quill);
  //       lines--;
  //     }
  //     setReadOnly(false);
  ...
  const handleChange1 = (content, delta, source, editor) => {
    ...
    deltaOps.forEach((op) => {
      if (op.insert && typeof op.insert === "string") {
       const stringWithoutSpaces = op.insert.replace(/\s/g, '');
       totalCharacters += stringWithoutSpaces.length;
       const newLines = op.insert.match(/\n/g);
        lines += newLines ? newLines.length : 0;
      }
    });
          ...
          <ReactQuill
            ...
            modules={{
              toolbar: {
                ...
                delay: 500, // time in milliseconds to wait before capturing the input and updating the editor
                maxStack: 5000, // limit on the number of history events stored
                userOnly: false // whether history events should only include changes made by the user
              }
            }}
          />          <div>
